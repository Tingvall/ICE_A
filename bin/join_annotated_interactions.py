...
argParser.add_argument('-p', '--prefix', dest="PREFIX", help="Prefix for output file.")
argParser.add_argument('-b', '--binsize', dest="BINSIZE", help="Interaction bin size.", type=int)

args = argParser.parse_args()

# DEFINE FUNCTION
def join_annotated_interactions(anchor1,anchor2,bed2D, prefix, binsize):
    anchor1_anno = pd.read_table(anchor1, index_col=0).sort_index()
    anchor2_anno = pd.read_table(anchor2, index_col=0).sort_index()
    bed2D = pd.read_table(bed2D,index_col=0).sort_index()
...
# RUN FUNCTION
join_annotated_interactions(anchor1=args.ANCHOR1_ANNO,anchor2=args.ANCHOR2_ANNO,bed2D=args.BED2D, prefix=args.PREFIX, binsize=args.BINSIZE)
