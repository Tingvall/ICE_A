...
def interaction_annotation_multiple(anchor_1_peak_collect, anchor_2_peak_collect, bed2D_index_anno, prefix, network, complete, binsize, promoter_start, promoter_end, upset_plot, circos_plot, circos_use_promoters, in_regions):
    ...
    anchors_peaks_anno=anchors_peaks_anno.sort_values(by=['Interaction_score']).drop_duplicates(subset=['Peak1_ID', 'Peak2_ID','Gene_Name_1', 'Gene_Name_2','Peak1', 'Peak2'], keep='first').sort_index()
    anchors_peaks_anno.index.name = 'Interaction'
    ...
    for f in factor:
        ...
        if (circos_use_promoters == "true"):
            factor_dict[f] = factor_dict[f].loc[((factor_dict[f]["Peak1_score"]==1) & (factor_dict[f]["Peak2_score"]==0)) | ((factor_dict[f]["Peak1_score"]==0) & (factor_dict[f]["Peak2_score"]==1)),:]
        factor_dict[f] = factor_dict[f].groupby(['Interaction', 'Gene_Name_1', 'Gene_Name_2']).agg(lambda x: ', '.join(filter(None, list(x.unique().astype(str)))))
    ...
    if (in_regions !="Not_specified" and circos_use_promoters == "true"):
        anchors_peaks_anno_promoters = anchors_peaks_anno.groupby(['Interaction', 'Gene_Name_1', 'Gene_Name_2']).agg(lambda x: ', '.join(filter(None, list(x[x!="REGIONS"].unique().astype(str)))))
        anchors_peaks_anno_promoters.to_csv(prefix + '_HOMER_annotated_interactions_with_peak_and_promoter.txt', index=True, sep='\t' )
        anchors_peaks_anno_filt = anchors_peaks_anno.loc[((anchors_peaks_anno["Peak1_score"]==1) & (anchors_peaks_anno["Peak2_score"]==0)) | ((anchors_peaks_anno["Peak1_score"]==0) & (anchors_peaks_anno["Peak2_score"]==1)),:]
        anchors_peaks_anno_filt = anchors_peaks_anno_filt.groupby(['Interaction', 'Gene_Name_1', 'Gene_Name_2']).agg(lambda x: ', '.join(filter(None, list(x[x!="REGIONS"].unique().astype(str)))))
