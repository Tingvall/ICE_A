...
process ANNOTATE_INTERACTION_WITH_PEAKS {
  ...
  if (params.mode == 'basic')
    ...
    # Column names for loaded data
    anchor1_peak_name = ('Anchor1_Chr', 'Anchor1_Start', 'Anchor1_End', 'Peak1_Chr', 'Peak1_Start', 'Peak1_End', 'Peak1_ID',)
    anchor2_peak_name = ('Anchor2_Chr', 'Anchor2_Start', 'Anchor2_End', 'Peak2_Chr', 'Peak2_Start', 'Peak2_End', 'Peak2_ID',)
    ...
    anchor1_peaks_anno =bed2D_anno.loc[:,['chr1', 's1','e1' ,'Entrez_ID_1', 'Gene_Name_1']].merge(anchor1_peaks.loc[:,['Peak1_ID']], left_index=True, right_index=True, how = 'left')
    anchor2_peaks_anno =bed2D_anno.loc[:,['chr2', 's2','e2' ,'Entrez_ID_2', 'Gene_Name_2','Q-Value_Bias','TSS_1', 'TSS_2']].merge(anchor2_peaks.loc[:,['Peak2_ID']], left_index=True, right_index=True, how = 'left')
...
process NETWORK {
  ...
  if (params.mode == 'basic')
    ...
    #Loading input file
    anchors_peaks_anno = pd.read_table("${interactions_annotated}")

    # Aggregating interaction file to only incude one row per interaction
    interactions_anno = anchors_peaks_anno.iloc[:,np.r_[0:5,6:14, 15:17]].groupby(by=['chr1', 's1', 'e1', 'Entrez_ID_1', 'Gene_Name_1', 'chr2', 's2', 'e2','Entrez_ID_2', 'Gene_Name_2', 'Q-Value_Bias', 'TSS_1', 'TSS_2'], axis=0, as_index=False).max()
    ...
    Factor_Interaction = Factor_Interaction[['chr1', 's1', 'e1','Gene_Name_1', 'Peak1','Peak1_ID','chr2', 's2', 'e2',  'Gene_Name_2','Peak2','Peak2_ID','TSS_1', 'TSS_2']]
...
process UPSET_PLOT {
  ...
  path "Upset_plot_Promoter_genes.pdf" optional true into ch_upset_plot_promoter_genes
